...
namespace WindowsVirtualDesktopHelper {
	class App {
		...
		public uint PreviousVDDisplayNumber = 0;
		KeyboardHook KeyboardHooksJumpToPreviousDesktop = null;
		...
		public void SetupHotKeys() {
			...
			if(this.KeyboardHooksJumpToPreviousDesktop != null) {
				this.KeyboardHooksJumpToPreviousDesktop.Dispose();
				this.KeyboardHooksJumpToPreviousDesktop = null;
			}

			if (this.SettingsForm.UseHotKeyToJumpToPreviousDesktop()) {
				this.KeyboardHooksJumpToPreviousDesktop= new KeyboardHook();
				this.KeyboardHooksJumpToPreviousDesktop.KeyPressed += new EventHandler<KeyPressedEventArgs>(HotKeyPressed);
				ModifierKeys modifier = this.SettingsForm.HotKeysToJumpToDesktop();
				var keys = new List<Keys>() { 
					Keys.D0, 
					Keys.NumPad0
				...
				foreach (var key in keys) {
					this.KeyboardHooksJumpToPreviousDesktop.RegisterHotKey(modifier, key);
				}
			}
		}
		...
		public void HotKeyPressed(object sender, KeyPressedEventArgs e) {
			...
			else if (e.Key == Keys.D0 || e.Key == Keys.NumPad0) desktopNumber = (int) this.PreviousVDDisplayNumber + 1;
			...
			while (true) {
				try {
					...
					if (newVDDisplayNumber != this.CurrentVDDisplayNumber) {
						...
						this.PreviousVDDisplayNumber = this.CurrentVDDisplayNumber;
